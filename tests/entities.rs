use crafter_rs::{Direction, Env, EnvConfig, Material};
...
fn achievements_affect_reward_once_and_length_can_end_episode() {
    ...
    let first = env.step_by_name("do").unwrap();
    assert!(first.reward >= 1.0);
    let collect_wood_idx = env.rules().achievement_index("collect_wood");
    assert!(first.info.achievements.count(collect_wood_idx) > 0);

    env.player_mut().set_facing(Direction::Right);
    env.world_mut().set_material(front, Material::Tree.id());
    let second = env.step_by_name("do").unwrap();
...
fn zombie_and_skeleton_updates_affect_player_and_world() {
    ...
    let zombie_id = env.rules().entity_type_id("zombie").unwrap();
    let zhealth = env.rules().entity_def(zombie_id).health;
    env.world_mut().spawn_entity([player[0] + 1, player[1]], zombie_id, zhealth);
    let health_before = env.player().health();
    let mut zombie_rewards = Vec::new();
    for _ in 0..10 {
        let zombie_step = env.step_by_name("noop").unwrap();
    ...
    env.world_mut().spawn_entity([player[0] + 4, player[1]], skel_id, shealth);
    let arrow_id = env.rules().entity_type_id("arrow").unwrap();
    for _ in 0..20 {
        env.step_by_name("noop").unwrap();
        if env.world().entity_count(arrow_id) > 0 {
            break;
        }
    }
    assert!(env.world().entity_count(arrow_id) > 0);
}
...
fn chunk_balancing_can_spawn_skeletons_in_populated_path_chunks() {
    ...
    let player = env.player_position();
    let cow_id = env.rules().entity_type_id("cow").unwrap();
    let cow_health = env.rules().entity_def(cow_id).health;
    for chunk_x in (0..64).step_by(12) {
        for chunk_y in (0..64).step_by(12) {
            let pos = [(chunk_x + 1).min(63), (chunk_y + 1).min(63)];
            if pos != player {
                env.world_mut().spawn_entity(pos, cow_id, cow_health);
            }
        }
    }
...
fn chunk_balancing_can_spawn_zombies_in_populated_grass_chunks() {
    ...
    let player = env.player_position();
    let plant_id = env.rules().entity_type_id("plant").unwrap();
    let plant_health = env.rules().entity_def(plant_id).health;
    for chunk_x in (0..64).step_by(12) {
        for chunk_y in (0..64).step_by(12) {
            let pos = [(chunk_x + 1).min(63), (chunk_y + 1).min(63)];
            if pos != player {
                env.world_mut().spawn_entity(pos, plant_id, plant_health);
            }
        }
    }
...
fn lethal_damage_marks_done_and_zero_discount() {
    ...
    let zhealth = env.rules().entity_def(zombie_id).health;
    env.world_mut().spawn_entity([player[0] + 1, player[1]], zombie_id, zhealth);
    let mut result = env.step_by_name("noop").unwrap();
    for _ in 0..10 {
        if result.done {
            break;
        }
        result = env.step_by_name("noop").unwrap();
    }
    assert!(result.done);
